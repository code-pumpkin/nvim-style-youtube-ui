...
// @version      7.0.0
// @description  Mindful YouTube — calm, distraction-free, responsive across all devices.
// @author       codePumpkin
...
(function () {
    ...
    function injectStats() {
        ...
        if (title) {
            ...
            titleEl.id = "mindful-title-inject";
        ...
        const titleInject = document.getElementById("mindful-title-inject");
    ...
    function openPanel(panel) {
        ...
        // Inject close button for touch devices
        if (isTouchDevice()) {
            setTimeout(() => {
                const target = document.querySelector(panelScrollTargets[panel]);
                if (target && !target.querySelector(".mindful-close-btn")) {
                    const btn = document.createElement("button");
                    btn.className = "mindful-close-btn";
                    btn.textContent = "✕ CLOSE";
                    Object.assign(btn.style, {
                        position:"sticky", top:"0", right:"0", zIndex:"99999",
                        display:"block", marginLeft:"auto", marginBottom:"8px",
                        background:C.bgDark, color:C.red, border:`1px solid ${C.red}`,
                        fontFamily:'"JetBrains Mono",monospace', fontSize:"11px",
                        padding:"8px 16px", cursor:"pointer", minHeight:"44px",
                    });
                    btn.addEventListener("click", e => { e.preventDefault(); e.stopPropagation(); closePanel(); });
    ...
    function closePanel() {
        ...
        // Remove touch close buttons
        document.querySelectorAll(".mindful-close-btn").forEach(el => el.remove());
    ...
    function updateBar() {
        ...
        if (state.mode === "LEADER") {
            ...
            barHints.textContent = isTouchDevice() ? "swipe to scroll · tap ✕ to close" : "Ctrl+j/k scroll  Esc close";
        } else if (isChannel()) {
            barHints.textContent = isTouchDevice() ? "tap tabs to navigate" : "1 videos  2 playlists  3 live  4 home  , leader";
        } else if (isWatch()) {
            barHints.textContent = isTouchDevice() ? "use bar buttons ↗" : ", leader  \\ back  H home";
        } else {
            barHints.textContent = isTouchDevice() ? "breathe · focus · enjoy" : ", leader  \\ back  H home";
        }
    ...
    function injectSearchBanner() {
        document.getElementById("mindful-search-banner")?.remove();
        if (!isSearch()) return;
        const q = new URLSearchParams(location.search).get("search_query");
        if (!q) return;

        const banner = document.createElement("div");
        banner.id = "mindful-search-banner";
    ...
    function init() {
        ...
        document.addEventListener("touchmove", e => {
            // Allow scrolling inside panels and normal scroll areas
            const inPanel = e.target.closest("[class*='mindful-panel']") ||
                            e.target.closest("#secondary") ||
                            e.target.closest("#comments") ||
                            e.target.closest("#above-the-fold") ||
                            e.target.closest("ytd-live-chat-frame");
            if (!inPanel && document.documentElement.scrollTop === 0) {
                // Only prevent if at top of page (pull-to-refresh zone)
            }
        }, { passive: true });

        // Fix: prevent clicks on video items from triggering navigation refresh on mobile
        // YouTube's SPA sometimes double-fires on touch — debounce navigation
        if (isTouchDevice()) {
            let lastNav = 0;
        ...
        document.addEventListener("fullscreenchange", () => {
            ...
            document.documentElement.style.setProperty("--bar-h", fs ? "0px" : (isMobile() ? "40px" : "32px"));
        });

        // Responsive bar height on resize
        window.addEventListener("resize", () => {
            if (!document.fullscreenElement) {
                document.documentElement.style.setProperty("--bar-h", isMobile() ? "40px" : "32px");
            }
            updateBar();
        });
